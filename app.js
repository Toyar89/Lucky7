...
// Run warm-up on FIRST pointer event in CAPTURE phase (before card handlers)
...
// Ultra-short fallback click (first flip only, if mp3 fails)
...
function startGame() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // pointerdown so first tap isn't lost to warm-up
...
function handleTurn(index, cardElement, backElement) {
  ...
  // Clear prompt but keep processing this tap
  const statusEl = document.getElementById('status');
  if (statusEl && statusEl.textContent) statusEl.textContent = '';

  // Double-ensure warm-up in this user gesture
...
function showHowToPlay(){
  ...
  if (m) { m.classList.add("show"); m.setAttribute("aria-hidden","false"); m.style.display = "block"; }
}
...
/* =======================
   Service worker (leave as you have it)
   ======================= */
